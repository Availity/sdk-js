...
expect.extend({
  toBeAuthorized(permissions, ids) {
    ...
    return {
      ...
      message: () => {
        return `expected ids (${this.utils.printExpected(ids)}) to ${this.isNot ? ' not ' : ''} have isAuthorized in permissions (${this.utils.printReceived(permissions)})`;
      }
    };
  }
});
...
describe('AvAuthorizations', () => {
  ...
  describe('Add permissions to map', () => {
    ...
    test('permission should be stored by region when one is provided', () => {
      ...
      expect(TestAuthorizations.authorizedMap[testId].FL.geographies).toEqual(testPermissionFl.geographies);
      expect(TestAuthorizations.authorizedMap[testId].GA.id).toBe(testId);
      expect(TestAuthorizations.authorizedMap[testId].GA.geographies).toEqual(testPermissionGa.geographies);
    });
  ...
  test('getFromMap should return permissions with ids in map based on region', () => {
    ...
    TestAuthorizations.addPermissions(allIds, [], region1);
    TestAuthorizations.addPermissions(addedIds, [], region2);
    expect(TestAuthorizations.getFromMap(allIds, region2)).toEqual(getMockPermissionValues(addedIds, false));
  });
  ...
  test('getPermission should reject when a non-string is passed in', () => {
    const testId = 123;
    return TestAuthorizations.getPermission(testId)
    .catch(err => {
  ...
  describe('Authorized', () => {
    ...
    test('should return true for authorized permission', done => {
      const testId = '123';
      const testRegion = 'GA';
      TestAuthorizations.isAuthorized(testId, testRegion)
      .then((isAuthorized) => {
        expect(mockPermissions.getPermissions).lastCalledWith([testId], testRegion);
    ...
    test('isAnyAuthorized should return true when all permissions are authorized', done => {
      const testIds = ['123', '456'];
      const testRegion = 'GA';
      TestAuthorizations.isAnyAuthorized(testIds, testRegion)
      .then((isAuthorized) => {
        expect(mockPermissions.getPermissions).lastCalledWith(testIds, testRegion);
        expect(isAuthorized).toBeTruthy();
        done();
      });
    });
    ...
    test('isAnyAuthorized should return true when some permissions are authorized', done => {
      ...
      .then(() => {
        expect(mockPermissions.getPermissions).lastCalledWith(testIdsFalse, testRegion);
        return TestAuthorizations.getPermissions(testIdsTrue, testRegion);
      })
      .then(() => {
        expect(mockPermissions.getPermissions).lastCalledWith(testIdsTrue, testRegion);
  ...
  test('getOrganizations should return organizations for permission', () => {
    ...
    .then((orgs) => {
      expect(mockPermissions.getPermissions).lastCalledWith([testId2], testRegion);
